...
- **Docker Image**: Single image bundles app + Ollama + n8n + reports server, all managed by systemd.
...
### Docker (Recommended)
See [INSTALLATION_LINUX.md](INSTALLATION_LINUX.md) or [INSTALLATION_WINDOWS.md](INSTALLATION_WINDOWS.md).

```bash
docker pull ghcr.io/galgo2077/eggshell:latest
docker run -it --rm -p 5678:5678 -p 8080:8080 --privileged ghcr.io/galgo2077/eggshell:latest
# inside container:
eggshell
```
...
├── docs/                     # this file, diagrams, install guides
├── reports/                  # charts/, results/, errors/ (git-ignored content)
├── systemd/                  # ollama.service, n8n.service, reports-server.service
├── requirements.txt
├── Dockerfile
└── src/
...
|---|---|---|
| Ollama | 11434 | Local AI inference (`qwen2.5:0.5b`, `0xroyce/plutus:latest`) |
| n8n | 5678 | Webhook workflow automation (Sentiment_proxy) |
| Reports server | 8080 | Browse HTML charts at `http://localhost:8080` |
